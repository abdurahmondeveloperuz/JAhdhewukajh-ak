...
@dp.callback_query_handler(state='films_data', text='author')
async def EnterName(call: CallbackQuery, state: FSMContext):
    await call.message.delete()
    await call.message.answer("✍️ Kino muallifini kiriting:",reply_markup=GoToAdminPanel)
    await state.set_state('films_author')

@dp.message_handler(state='films_author')
async def adminPanelfunc(message: Message, state: FSMContext):
    author = message.text
    await state.update_data({f"{message.from_user.id}_author":author})
    await message.answer("""<b>
✅Kino muallifi saqlandi
🤘Qolgan ma'lumotlarni kiritishingiz
🔸Yoki tugatishingiz ham mumkin</b>""",reply_markup=about_film_keyboards)
    await state.set_state("films_data")

@dp.callback_query_handler(state='films_data', text='year')
async def EnterName(call: CallbackQuery, state: FSMContext):
    await call.message.delete()
    await call.message.answer("📅 Kino ishlab chiqilgan yilni kiriting:",reply_markup=GoToAdminPanel)
    await state.set_state('films_year')

@dp.message_handler(state='films_year')
async def adminPanelfunc(message: Message, state: FSMContext):
    year = message.text
    await state.update_data({f"{message.from_user.id}_year":year})
    await message.answer("""<b>
✅Kino chiqarilgan yil saqlandi
🤘Qolgan ma'lumotlarni kiritishingiz
🔸Yoki tugatishingiz ham mumkin</b>""",reply_markup=about_film_keyboards)
    await state.set_state("films_data")
...
async def adminPanelfunc(message: Message, state: FSMContext):
    ...
    await message.answer("""<b>
✅Kino ishlab chiqarilgan davlat saqlandi
🤘Qolgan ma'lumotlarni kiritishingiz
🔸Yoki tugatishingiz ham mumkin</b>""",reply_markup=about_film_keyboards)
    await state.set_state("films_data")
...
async def EnterName(call: CallbackQuery, state: FSMContext):
    await call.message.delete()
    state_data = await state.get_data(f"{call.from_user.id}_FileId")
    kino_file_id = state_data.get(f"{call.from_user.id}_FileId")
    try: 
        name = await state.get_data(f"{call.from_user.id}_name")
        name = name.get(f"{call.from_user.id}_name")
    except:
        name = False
    try: 
        author = await state.get_data(f"{call.from_user.id}_author")
        author = author.get(f"{call.from_user.id}_author")

    except:
        author = False
    try: 
        year = await state.get_data(f"{call.from_user.id}_year")
        year = year.get(f"{call.from_user.id}_year")
    except:
        year = False
    try: 
        country = await state.get_data(f"{call.from_user.id}_country")
        country = country.get(f"{call.from_user.id}_country")
    except:
        country = False
    try: 
        language = await state.get_data(f"{call.from_user.id}_language")
        language = language.get(f"{call.from_user.id}_language")
    except:
        language = False
    try: 
        quality = await state.get_data(f"{call.from_user.id}_quality")
        quality = quality.get(f"{call.from_user.id}_quality")
    except:
        quality = False
    try: 
        continuity = await state.get_data(f"{call.from_user.id}_continuity")
        continuity = continuity.get(f"{call.from_user.id}_continuity")
    except:
        continuity = False
    try: 
        genre = await state.get_data(f"{call.from_user.id}_genre")
        genre = genre.get(f"{call.from_user.id}_genre")
    except:
        genre = False
    try: 
        size = await state.get_data(f"{call.from_user.id}_size")
        size = genre.get(f"{call.from_user.id}_size")
    except:
        size = False
    caption = ""
    if name:
        caption+=f"🎬 Nomi: {name}\n➖➖➖➖➖➖➖\n"
    if author:
        caption+=f"🖋Muallif: {author}\n"        
    if year:
        caption+=f"🎥 Yili: {year}\n"
    if country:
        caption+=f"🌐 Davlat: {country}\n"
    if language:
        caption+=f"🖌 Til: {language}\n"
    if quality:
        caption+=f"💿 Sifat: {quality}\n"
    if size:
        caption += f"📀 Hajmi: {size}\n"
    if continuity:
        caption+=f"⏰ Davomkiylik: {continuity}\n"
    if genre:
        caption+=f"🎭 Janr: {genre}\n"


    caption+="\n\n✌️@SaraFilmUzBot - 🎬Eng zo'r kinolar faqat bizda!"
...
async def adminPanelfunc(call: CallbackQuery, state: FSMContext):
    ...
    try: 
        size = await state.get_data(f"{call.from_user.id}_size")
        size = size.get(f"{call.from_user.id}_size")
    except:
        size = False
    state_data = await state.get_data(f"{call.from_user.id}_kod")
    kino_kod = state_data.get(f"{call.from_user.id}_kod")
    caption=f"<b>✅Kino tasdiqlandi va kino ro'yhatga qo'shildi\n🆔Kino kodi: {kino_kod}\n🔗Kino ssilkasi: https://t.me/SaraFilmUzBot?start={kino_kod}</b>"
    try:
        films_db.save_film(code=kino_kod,name=name,genre=genre,file_id=kino_file_id,size=size,author=author,continuity=continuity,from_country=country,lang=language,quality=quality)
