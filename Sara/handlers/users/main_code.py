...
async def handle_inline_query(query: InlineQuery):
    ...
    if query_text=="":
        ...
        id=code,
        # photo_url="https://i.imgur.com/2HEaBZj.jpg",
        thumb_url="https://i.imgur.com/2HEaBZj.jpg",
        title=title,
...
async def async_function(call: types.CallbackQuery,state: FSMContext):
    ...
    views = views_db.get_views_film(code=code)
    if name != "0" and name != None:
        caption+= f"🎬 Nomi: {name}\n"
    if genre != "0" and genre != None:
        caption += f"🎭 Janr: {genre}\n"
    if continuity != "0" and continuity!= None:
        caption += f"⏰ Davomiylik: {continuity}\n"
    if author != "0" and author!= None:
        caption += f"🎬 Nomi: {name}\n"
    if language != "0" and language!= None:
        caption += f"🖌 Til: {language}\n"
    if quality != "0" and quality!= None:
        caption += f"💿 Sifat: {quality}p\n"
    if size != "0" and size!= None:
        caption += f"📀 Hajmi: {size}\n"
    if from_country != "0" and from_country!= None:
        caption += f"🌐 Davlat: {from_country}\n"
    caption += f"👁Ko'rishlar soni: {views}\n\n\n"
    caption += f"✌️@SaraFilmUzBot - 🎬Eng zo'r kinolar faqat bizda!"
...
async def send_film(message: types.Message):
    ...
    if name != "0" and name:
        caption+= f"🎬 Nomi: {name}\n"
    if genre != "0" and genre:
        caption += f"🎭 Janr: {genre}\n"
    if continuity != "0" and continuity:
        caption += f"⏰ Davomiylik: {continuity}\n"
    if author != "0" and author:
        caption += f"✍🏻 Muallif: {author}\n"
    if language != "0" and language:
        caption += f"🖌 Til: {language}\n"
    if quality != "0" and quality:
        caption += f"💿 Sifat: {quality}p\n"
    if size != "0" and size:
        caption += f"📀 Hajmi: {size}\n"
    if from_country != "0" and from_country:
        caption += f"🌐 Davlat: {from_country}\n"
    caption += f"👁Ko'rishlar soni: {views}\n\n\n"
    caption += f"✌️@SaraFilmUzBot - 🎬Eng zo'r kinolar faqat bizda!"
